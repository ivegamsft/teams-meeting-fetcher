...
Write-Host "1. cd iac/azure && terraform plan && terraform apply" -ForegroundColor Gray
Write-Host "2. Run the Teams admin bootstrap (separate team/role):" -ForegroundColor Gray
Write-Host "   .\scripts\bootstrap-teams-policies.ps1 -AppId `"$appId`" -TenantId `"$tenantId`"" -ForegroundColor Gray
Write-Host ""
