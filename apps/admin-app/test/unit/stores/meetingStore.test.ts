...
describe('meetingStore', () => {
  ...
  describe('put', () => {
    test('saves meeting to DynamoDB', async () => {
      ...
      expect(call.input.Item).toEqual(mockMeeting);
  ...
  describe('get', () => {
    test('returns meeting when found', async () => {
      mockSend.mockResolvedValue({ Item: mockMeeting });
  ...
  describe('list', () => {
    test('returns paginated meetings sorted by startTime desc', async () => {
      const m1 = { ...mockMeeting, id: 'm1', startTime: '2025-07-10T10:00:00Z' };
      const m2 = { ...mockMeeting, id: 'm2', startTime: '2025-07-11T10:00:00Z' };
      mockSend.mockResolvedValue({ Items: [m1, m2] });

      const result = await meetingStore.list();
      expect(result.totalCount).toBe(2);
      expect(result.meetings[0].id).toBe('m2');
      expect(result.meetings[1].id).toBe('m1');
    });
  ...
  describe('updateStatus', () => {
    test('updates meeting status', async () => {
      ...
      const call = mockSend.mock.calls[0][0];
      expect(call.input.Key).toEqual({ id: 'meeting-1' });
  ...
  describe('setTranscriptionId', () => {
    test('sets transcription id and status', async () => {
      ...
      const call = mockSend.mock.calls[0][0];
      expect(call.input.ExpressionAttributeValues[':tid']).toBe('transcript-1');
      expect(call.input.ExpressionAttributeValues[':status']).toBe('transcript_pending');
    });
  });
});
