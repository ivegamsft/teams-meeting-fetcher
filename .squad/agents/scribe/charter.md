...
- `.squad/orchestration-log/` — per-spawn orchestration entries
...
2. **Write orchestration log entries** to `.squad/orchestration-log/{timestamp}-{agent}.md`:
   - One entry per agent that was spawned
   - Records: agent routed, why chosen, mode, files authorized, files produced, outcome
...
4. **Deduplicate and consolidate decisions.md:**
   ...
   - **Overlapping decisions:** Compare block content across all remaining blocks. If two or more blocks cover the same area, consolidate them into a single merged block.
...
7. **History summarization:** If any agent's `history.md` exceeds ~12KB, summarize old entries under `## Core Context` and archive details.

8. **Decisions archive:** If `decisions.md` exceeds ~20KB, archive entries older than 30 days to `decisions-archive.md`.
...
└── agents/
    ├── keaton/history.md   # Keaton's personal knowledge
    ├── mcmanus/history.md  # McManus's personal knowledge
    ├── fenster/history.md  # Fenster's personal knowledge
    ├── hockney/history.md  # Hockney's personal knowledge
    └── scribe/history.md   # Scribe's personal knowledge
```

## Boundaries

**I handle:** Logging, memory, decision merging, orchestration logs, cross-agent updates.
